...
/// already right for later phases; only the MVP buttons have handlers wired up
/// in <see cref="PlanFlowRibbon"/> today (the rest call a "not available yet in
/// this phase" stub so nothing in the UI is silently a no-op).
/// </summary>
    ...
    <tabs>
      <tab id='planFlowTab' label='PlanFlow Pro'>
        ...
        <group id='grpAssistant' label='Assistant'>
          ...
          <button id='btnSuggestLogic' label='Suggest Logic' onAction='OnNotImplemented' size='large' />
        </group>
